...
The framework, install, build, and output settings are in [vercel.json](../vercel.json). The Node major is enforced by `package.json` and the build; root-directory selection and the absence of secrets or integrations require dashboard verification. Confirm that dashboard overrides do not contradict the checked-in settings. The build needs no test dependencies; CI installs those separately. Review [Vercel configuration](https://vercel.com/docs/project-configuration/vercel-json) and [Node version selection](https://vercel.com/docs/functions/runtimes/node-js/node-js-versions) when platform behavior changes.
...
Before manual promotion—or before authorizing a merge that will trigger automatic production—verify the applicable pre-release items below. After any promotion, complete the deployment-specific items against the actual HTTPS deployment:

1. GitHub's candidate source and tree match the approved revision; its required checks succeeded. For Git-triggered production, also wait for the exact merge commit's checks and confirm its tree matches the reviewed head.
...
An automatic Git rollout can expose bytes before post-merge checks finish. It is not accepted until the exact merge checks and live smoke pass; rollback on failure. When repository and project settings changes are separately authorized, protect `main` with required checks and use [Vercel Deployment Checks](https://vercel.com/docs/deployment-checks) to hold alias promotion until its selected gates pass.
